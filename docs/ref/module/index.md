...
|  [**`MINIO`**](/docs/minio)  | Extension |   GA   | `/docs/minio`  | Deploys Silo S3-compatible object storage, suitable for PostgreSQL backups.                            |
...
- [**`PGSQL`**](/docs/pgsql): Self-healing PostgreSQL clusters with HA, PITR, IaC, SOP, monitoring, and [**576**](/ext/list) extensions.
...
- [**`MINIO`**](/docs/minio): An S3-compatible object-storage module that deploys Silo and provides PostgreSQL backup integration and monitoring.
...
- [**Native `MYSQL` pilot**](/docs/mysql/): the current `mysql.yml`, `mysql-rm.yml`, and `roles/mysql*` manage a fixed native MySQL 8.4 platform with either one node or a three-node single-primary InnoDB Cluster. It remains a PILOT and is not counted among the 10 official modules above.
