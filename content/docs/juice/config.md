...
In Pigsty, metadata is always PostgreSQL (`meta` URL), while data storage is defined by `data` options passed to `juicefs format`.
...
- `data` is required on **first format**, otherwise filesystem cannot be created.
