...
weight: 595
description: Deploy Mattermost on Pigsty v4.3 and store state in external PostgreSQL.
module: [SOFTWARE]
...
Pigsty v4.3 provides `app/mattermost` (`conf/app/mattermost.yml`), which stores app state in external PostgreSQL and persists file directories on host paths.
