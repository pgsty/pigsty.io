...
weight: 645
description: Self-host Electric on Pigsty v4.3 to sync PostgreSQL data to frontend apps with partial replication and real-time delivery.
module: [SOFTWARE]
...
Pigsty v4.3 provides the `app/electric` template (`conf/app/electric.yml`) to bootstrap database, container, and ingress settings in one flow.
