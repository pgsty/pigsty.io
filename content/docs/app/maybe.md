...
weight: 600
description: Self-host Maybe on Pigsty v4.4 with external PostgreSQL storage.
module: [SOFTWARE]
...
Pigsty v4.4 provides the `app/maybe` template (`conf/app/maybe.yml`) to run Maybe as stateless containers while storing business data in external PostgreSQL.
