...
- [**Native `MYSQL` pilot**](/docs/pilot/mysql/): the current `mysql.yml`, `mysql-rm.yml`, and `roles/mysql*` manage a fixed native MySQL 8.4 platform with either one node or a three-node single-primary InnoDB Cluster. It remains a PILOT and is not counted among the 11 official modules above.
