...
The current `main` branch (v4.5.0 development) provides 52 distinct YAML templates under `conf/` for deployment, testing, and package-build scenarios. The latest stable release, v4.4.0, provides 48; the additions are `demo/kafka`, `demo/mysql`, `demo/rustfs`, and `ha/octo`. The `app/supa.yml` compatibility symlink to `supabase.yml` is not counted twice.
