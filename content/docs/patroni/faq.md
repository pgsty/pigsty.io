...
They very first consequence is that all the Patroni clusters that rely on that DCS will go to read-only mode -- unless [dcs_failsafe_mode](/docs/patroni/dcs_failsafe_mode#dcs_failsafe_mode) is enabled.
...
You can use the [dcs_failsafe_mode](/docs/patroni/dcs_failsafe_mode#dcs_failsafe_mode) to alleviate that situation.

--------

## patronictl

Do I need to run [patronictl](/docs/patroni/patronictl#patronictl) in the Patroni host?  
No, you do not need to do that.

Running [patronictl](/docs/patroni/patronictl#patronictl) in the Patroni host is handy if you have access to the Patroni host because you can use the very same configuration file from the `patroni` agent for the [patronictl](/docs/patroni/patronictl#patronictl) application.

However, [patronictl](/docs/patroni/patronictl#patronictl) is basically a client and it can be executed from remote machines. You just need to provide it with enough configuration so it can reach the DCS and the REST API of the Patroni member(s).

Why did the information from one of my Patroni members disappear from the output of [patronictl_list](/docs/patroni/patronictl#patronictl_list) command?  
Information shown by [patronictl_list](/docs/patroni/patronictl#patronictl_list) is based on the contents of the DCS.
...
As the member is not able to update the information, the information eventually expires from the DCS, and consequently the member is not shown anymore in the output of [patronictl_list](/docs/patroni/patronictl#patronictl_list).

Why is the information about one of my Patroni members not up-to-date in the output of [patronictl_list](/docs/patroni/patronictl#patronictl_list) command?  
Information shown by [patronictl_list](/docs/patroni/patronictl#patronictl_list) is based on the contents of the DCS.
...
See more in [config](/docs/patroni/config#config).
...
Please refer to [generate_sample_config](/docs/patroni/config#generate_sample_config) and [generate_config](/docs/patroni/config#generate_config) for more details.
...
- [patronictl_edit_config](/docs/patroni/patronictl#patronictl_edit_config); or
- A `PATCH` request to [config_endpoint](/docs/patroni/rest_api#config_endpoint).
...
- Send a `POST` request to the REST API [reload_endpoint](/docs/patroni/rest_api#reload_endpoint); or

- Run [patronictl_reload](/docs/patroni/patronictl#patronictl_reload); or
...
**Note:** there are cases where a reload through the [patronictl_reload](/docs/patroni/patronictl#patronictl_reload) may not work:

- Expired REST API certificates: you can mitigate that by using the `-k` option of the [patronictl](/docs/patroni/patronictl#patronictl);
- Wrong credentials: for example when changing `restapi` or `ctl` credentials in the configuration file, and using that same configuration file for Patroni and [patronictl](/docs/patroni/patronictl#patronictl).
...
Take care to not cause a failover in the cluster! You might be interested in checking [patronictl_pause](/docs/patroni/patronictl#patronictl_pause).
...
You can set it either in the Patroni YAML file ([log settings](/docs/patroni/config/yaml#log_settings)) or with `PATRONI_LOG_DEDUPLICATE_HEARTBEAT_LOGS=true`.
...
- [patronictl_restart](/docs/patroni/patronictl#patronictl_restart); or
- A `POST` request to [restart_endpoint](/docs/patroni/rest_api#restart_endpoint).

**Note:** some Postgres GUCs require a special management in terms of the order for restarting the Postgres nodes. Refer to [shared_memory_gucs](/docs/patroni/config#shared_memory_gucs) for more details.
...
**Note:** when modifying these settings, please keep in mind that Patroni enforces the rule and minimal values described in [dynamic](/docs/patroni/config/dynamic#dynamic) section of the docs.
...
You can find more information about that in the section [important_configuration_rules](/docs/patroni/config#important_configuration_rules).
...
Please refer to [existing_data](/docs/patroni/existing_data#existing_data) for detailed instructions.
...
- [patronictl](/docs/patroni/patronictl#patronictl): This is a command-line utility used to interact with a Patroni cluster (perform switchovers, restarts, changes in the configuration, etc.). Please find more information in [patronictl](/docs/patroni/patronictl#patronictl).
...
Refer to [standby_cluster](/docs/patroni/standby_cluster#standby_cluster) for more details.
...
What does [pause](/docs/patroni/pause#pause) mean in Patroni?  
...
You can find more information about that in [pause](/docs/patroni/pause#pause).
...
For detailed information please refer to [custom_bootstrap](/docs/patroni/replica_bootstrap#custom_bootstrap) and [custom_replica_creation](/docs/patroni/replica_bootstrap#custom_replica_creation).
...
Patroni exposes a couple handy endpoints in its [rest_api](/docs/patroni/rest_api#rest_api):
