...
- How to enable the Failsafe Mode?
  ...
  Before enabling the `failsafe_mode` please make sure that Patroni version on all members is up-to-date. After that, you can use either the `PATCH /config` [REST API](/docs/patroni/rest_api#rest_api) or [patronictl edit-config -s failsafe_mode=true](/docs/patroni/patronictl#patronictl_edit_config_parameters)
