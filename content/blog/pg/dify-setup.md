---
title: Self-Hosting Dify with PG, PGVector, and Pigsty
date: 2024-06-22
manual_link: "https://vonng.com/en/pg/dify-setup/"
author: |
  [Ruohang Feng](https://vonng.com/en/) ([@Vonng](https://vonng.com/en/)) | [WeChat Column](https://mp.weixin.qq.com)

description: |
  Dify is an open-source LLM app development platform. This article explains how to self-host Dify using Pigsty.

images: [/img/hero/pg/dify-setup.jpg]
tags: [PostgreSQL, Docker, PigstyApp]
---

[![featured](/img/hero/pg/dify-setup.jpg)](https://vonng.com/en/pg/dify-setup/)

Dify is an open-source LLM app development platform. This article explains how to self-host Dify using Pigsty. [**Read more**](https://vonng.com/en/pg/dify-setup/)
