...
readonly BASEURL="https://repo.pigsty.io"

# To install the latest version of pig
# curl -fsSL https://repo.pigsty.io/pig | bash

# To install a specific version of pig
# curl -fsSL https://repo.pigsty.io/pig | bash -s 1.8.0
...
#--------------------------------------------------------------#
# Check whether the target package is already installed
#--------------------------------------------------------------#
package_installed() {
    case "${OS_PACKAGE}" in
        rpm)
            rpm -q --quiet "pig-${VERSION}-1.${ARCH}"
            ;;
        deb)
            local installed
            installed=$(dpkg-query -W -f='${Status} ${Version}' pig 2>/dev/null) || return 1
            [[ "${installed}" == "install ok installed ${VERSION}-1" ]]
            ;;
    esac
}
...
main() {
    ...
    if package_installed; then
        log_warn "pig v${VERSION} is already installed, skip installation"
        return 0
    fi
